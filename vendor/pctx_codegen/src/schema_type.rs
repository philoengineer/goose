    ...
    pub fn type_signature(
        ...
        let mut sig: String = match self {
            ...
            SchemaType::Array(ArraySchemaType { item_schema, .. }) => {
                let item_sig = SchemaType::from(item_schema).type_signature(true, defs)?;
                // Union item signatures must be parenthesized: `string | Obj[]`
                // parses as `string | (Obj[])` and rejects `["..."]`.
                if item_sig.contains('|') {
                    format!("({item_sig})[]")
                } else {
                    format!("{item_sig}[]")
                }
